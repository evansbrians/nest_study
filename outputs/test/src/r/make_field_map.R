...
source(
  here("outputs/test/src/r/app_functions.R")
)
...
map_mobile_friendly <-
  ...
  appendContent(
    tags$style(
      HTML(
        read_file(
          here("outputs/test/src/css/map_styles.css")
        )
      )
    )
  )
...
map_tracking <-
  ...
  onRender(
    read_file(
      here("outputs/test/src/js/map_tracking.js")
    )
  )
...
map_tracking <-
  map_tracking %>%
  onRender(
    read_file(here("outputs/test/src/js/map_weather.js"))
  )
  ...
  tryCatch(
    {
      ...
      source(
        here("outputs/test/src/r/schedule_for_map.R"),
        local = sched_env
      )
...
tile_cache_dir <- here("outputs/test/offline_tiles")
