...
source(
  here("outputs/nest_app_api/src/r/app_functions.R")
)

# basemap -----------------------------------------------------------------

basemap <-
  # attributionControl = FALSE drops Leaflet's default "Leaflet" attribution
  # link (the stray control in the map corner). zoomControl left on.
...
icons <-
  list.files(
    here("outputs/nest_app_api/icons"),
    pattern = "png$",
    full.names = TRUE
  ) %>% 
...
map_mobile_friendly <-
  ...
  appendContent(
    tags$style(
      HTML(
        read_file(
          here("outputs/nest_app_api/src/css/map_styles.css")
        )
      )
    )
  )
...
map_tracking <-
  ...
  onRender(
    read_file(
      here("outputs/nest_app_api/src/js/map_tracking.js")
    )
  )
...
map_tracking <-
  map_tracking %>%
  onRender(
    read_file(here("outputs/nest_app_api/src/js/map_weather.js"))
  )
  ...
  tryCatch(
    {
      ...
      source(
        here("outputs/nest_app_api/src/r/schedule_for_map.R"),
        local = sched_env
      )
...
# fieldNestFade / fieldNestBig are NO LONGER BAKED. Marker opacity + size now
# come from the v_map_point DB view (GET /map_points -> applyMapPointStyles),
# keyed by gps_point.point_id. The baked maps were keyed by "lat,lng" strings,
# which the id-based join can't match -- and keeping a rival copy is exactly how
...
tile_cache_dir <- here("outputs/nest_app_api/offline_tiles")
...
# the Waypoint manager can list and navigate to them. The base64 `photo` field is
# deliberately NOT baked here -- it was ~14 MB of the shell -- because the app now
# lazy-fetches each point's photo from GET /gps_points/<id>/photo on demand.
...
# Nest ids are no longer baked: the app derives them from GET /nests
# (fieldApiNests) for the next-nest-ID suggestion, with IndexedDB as the offline
# fallback. (Removed window.fieldNestIds bake -- static-shell refactor stage A.)
...
# Externalized shell payloads, written INTO the staging tree (nest_app_api).
# field_map.qmd loads these via <script src> resolved relative to itself, so
# writing them here keeps the embedded shell fresh. All source assets (R helpers,
# icons, css, tiles) are now read from this same staging tree too -- the retired
# outputs/nest_app app has been removed, so nothing depends on it any more.
shell_dir <- here("outputs/nest_app_api")

writeLines(
  c("/* field_patches.js */", field_patches_js),
  file.path(shell_dir, "field_patches.js")
)

writeLines(
  c("/* field_icons.js */", field_icons_js),
  file.path(shell_dir, "field_icons.js")
)

writeLines(
  c("/* field_offline_tiles.js */", field_offline_tiles_js),
  file.path(shell_dir, "field_offline_tiles.js")
)
...
writeLines(
  ...
  file.path(shell_dir, "field_data.js")
)
